 ...
 */
import { CapabilityDelegation, constants as zcapConstants } from '@interop/zcap'
  ...
  cipherVersions.forEach(cipherVersion => {
    ...
    describe(`"${cipherVersion}" cipher version`, () => {
      ...
      it('should insert a document with 5 recipients', async () => {
        ...
        const recipients = (
          await Promise.all([1, 2, 3, 4].map(async () => kak))
        ).map(createRecipient)
      ...
      it('should read a document using a delegated capability', async function () {
        ...
        const unsignedCapability = {
          '@context': [ZCAP_CONTEXT_URL, Ed25519Signature2020.CONTEXT_URL],
          id: `urn:uuid:${uuid()}`,
        ...
        const delegatedCapability = await sign(unsignedCapability, {
          documentLoader,
          suite,
          purpose
        })
