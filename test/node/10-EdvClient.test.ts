...
describe('EdvClient', () => {
  ...
  cipherVersions.forEach(cipherVersion => {
    ...
    beforeEach(() => {
      kak =
        cipherVersion === 'fips'
          ? mock.keys.fips.keyAgreementKey
          : mock.keys.keyAgreementKey
    })
    ...
    describe(`"${cipherVersion}" cipher version`, () => {
      ...
      it('should not find a deleted document via its attributes', async () => {
        ...
        const { documents: docsBefore } = await client.find({
          equals: { 'content.indexedKey': 'toDelete' },
          invocationSigner
        })
      ...
      it(
        'should not find a deleted document via its attributes when ' +
          'deleted with no hmac set',
        async () => {
          const client = await mock.createEdv({ cipherVersion })
          client.ensureIndex({ attribute: 'content.indexedKey' })
          const testId = await EdvClient.generateId()
      ...
      it(
        'should find document that has both fields in a ' + 'compound index',
        async () => {
          const client = await mock.createEdv({ cipherVersion })
          client.ensureIndex({ attribute: ['content.a', 'content.b'] })
          const testId = await EdvClient.generateId()
      ...
      it(
        'should find document when only first field in compound index ' +
          'is specified in "has" query',
        async () => {
          const client = await mock.createEdv({ cipherVersion })
          client.ensureIndex({ attribute: ['content.a', 'content.b'] })
          const testId = await EdvClient.generateId()
      ...
      it(
        'should not find document when only second field in compound index ' +
          'is specified in "has" query',
        async () => {
          const client = await mock.createEdv({ cipherVersion })
          client.ensureIndex({ attribute: ['content.a', 'content.b'] })
      ...
      it(
        'should find a document with a deep compound index ' + 'on an array',
        async () => {
          const client = await mock.createEdv({ cipherVersion })
          client.ensureIndex({
      ...
      it(
        'should find a document with a deep compound index ' + 'on arrays',
        async () => {
          const client = await mock.createEdv({ cipherVersion })
          client.ensureIndex({
      ...
      describe('migrate version 1 blinded attributes to version 2', () => {
        ...
        it(
          ...
          async () => {
            ...
            client.ensureIndex({
              attribute: [
                'content.nested.array.foo',
                'content.second.array.bar'
              ]
            })
            const testId = await EdvClient.generateId()
            const expected = {
