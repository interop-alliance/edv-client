...
export class TestMock {
  ...
  async init() {
    ...
    if (!this.keys) {
      ...
      this.keyStorage.set(
        this.keys.fips.keyAgreementKey.id,
        this.keys.fips.keyAgreementKey
      )
  ...
  async createEdv({
    ...
    const { keyAgreementKey } =
      cipherVersion === 'fips' ? this.keys.fips : this.keys
    let config = {
      sequence: 0,
    ...
    config = await EdvClient.createEdv({ config, url: `${BASE_URL}/edvs` })
